...
public:
    ...
    // ppPageLockedSysmemBuffers will receive dwNBuffers*2 pointers if eSysStereoFormat is set to a stereo mode, dwNBuffers pointers otherwise
    // ppTransferCompletionEvents will receive dwNBuffers event handles

    virtual NVIFRRESULT NvIFRSetUpTargetBufferToSys(NVIFR_TOSYS_SETUP_PARAMS *pParams)=0;

    // NvIFR_TransferRenderTargetToSys() call:
